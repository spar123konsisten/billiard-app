  ...
  const handleTerima = async (item) => {
    if (item.isGuest) {
      ...
      const res = await fetch('/api/pertandingan', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'terima', matchId: item.id }),
      });
      const result = await res.json();
  ...
  const handleTolak = async (item) => {
    if (item.isGuest) {
      ...
      const res = await fetch('/api/pertandingan', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'tolak', matchId: item.id }),
      });
      const result = await res.json();
      if (result.success && result.waLink) {
        window.location.href = result.waLink;
    ...
    <main style={{ maxWidth: '420px', margin: '0 auto', padding: '24px 16px 80px', fontFamily: 'sans-serif' }}>
      ...
      {activeTab === 'ajakan' && (
        <>
          ...
          {ajakanData.map(item => {
            const isGuest = item.isGuest || false;
            const opponentName = isGuest ? item.opponent.nama : item.opponent?.nama;
            const opponentFoto = isGuest ? null : item.opponent?.foto_url;
            const opponentTier = isGuest ? 'Guest' : item.opponent?.tier;
            const opponentBintang = isGuest ? 0 : item.opponent?.bintang;

            return (
              <div key={item.id} style={cardStyle}>
                <div style={{ display:'flex', gap:'12px', alignItems:'center' }}>
                  <img
                    src={opponentFoto || `https://picsum.photos/seed/${item.id || 'guest'}/56/56`}
                    alt={opponentName}
                    style={imgStyle}
                  />
                  <div style={{ flex:1 }}>
                    <div style={{ fontWeight:'600', fontSize:'16px', marginBottom:'4px' }}>
                      {isGuest ? '👤 Guest: ' : ''}{opponentName}
                    </div>
                    <div style={{ fontSize:'12px', color:'#666' }}>
                      {isGuest ? '' : opponentTier} {isGuest ? '' : '★'.repeat(opponentBintang)}
                      {isGuest && <span style={{ background: '#f0f0f0', padding: '2px 6px', borderRadius: '3px', fontSize: '10px', marginLeft: '6px' }}>Guest</span>}
                    </div>
                ...
                {item.userRole === 'challenger' && !isGuest && (
      ...
      {activeTab === 'terjadwal' && (
        <>
          ...
          {terjadwalData.map(item => {
            const isGuest = item.isGuest || false;
            const opponentName = isGuest ? item.opponent?.nama || 'Guest' : item.opponent?.nama;
            const opponentFoto = isGuest ? null : item.opponent?.foto_url;
            const opponentTier = isGuest ? 'Guest' : item.opponent?.tier;
            const opponentBintang = isGuest ? 0 : item.opponent?.bintang;
            ...
            return (
              <div key={item.id} style={cardStyle}>
                ...
                <div style={{ display:'flex', justifyContent:'flex-end', marginTop:'12px' }}>
                  <button onClick={() => handleInputSkor(item.id, opponentName)} style={{ padding:'8px 20px', background:'#000', color:'#fff', border:'none', borderRadius:'4px', cursor:'pointer', display:'flex', alignItems:'center', gap:'6px' }}>Input Skor <span>→</span></button>
                </div>
              </div>
            );
          })}
