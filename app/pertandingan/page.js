...
export default function PertandinganPage() {
  ...
  const checkExpired = async () => {
    try {
      ...
      // Setelah expired, refresh terjadwal
  ...
  useEffect(() => {
    ...
    if (activeTab === 'ajakan') {
      fetchAjakan();
      interval = setInterval(fetchAjakan, 30000); // 30 detik
    } else if (activeTab === 'terjadwal') {
      fetchTerjadwal();
      // Polling 5 menit: cek expired + refresh data
      ...
      fetchRiwayat();
      // Riwayat tidak ada interval (hanya fetch sekali saat tab dibuka)
    }
